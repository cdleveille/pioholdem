    ...
    class HandEvaluator
    {
        ...
        public int EvaluateHands(Player[] players, Card[] board)
        {
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                // Show the player's hole cards and hand rank
                Console.WriteLine(player.name + " shows |" + player.holeCards[0] + "|" + player.holeCards[1] + "| *" + HandRankToString(handValue) + "*");
        ...
        {
            // Find "paired values" that appear more than once within the hand.
            // These are not included in the calculated kicker value.
            int[] pairedValues = new int[hand.Length];
            int removedCount = 0;
            for (int i = 0; i < hand.Length; i++)
                foreach (Card card in hand)
                    if (hand[i].suit != card.suit && hand[i].value == card.value && !pairedValues.Contains(card.value) && removedCount < 2)
                    {
            ...
            foreach (Card card in hand)
                ...
                {
                    // Add 1 to the value so that 2s (value 0) are not weightless
        ...
        public string ClassifyHoleCards(Card[] holeCards)
        {
            ...
            if (sortedHoleCards[0].value != sortedHoleCards[1].value)
                holeCardString += sortedHoleCards[0].suit == sortedHoleCards[1].suit ? "s" : "o";
            return holeCardString;
        }

        // Get the string representation of the player's hand rank from the given hand value
        private string HandRankToString(int handValue)
        {
            if (handValue == 800000012)
                return "Royal Flush";
            else if (handValue >= 800000000)
                return "Straight Flush";
            else if (handValue >= 700000000)
                return "Four of a Kind";
            else if (handValue >= 600000000)
                return "Full House";
            else if (handValue >= 500000000)
                return "Flush";
            else if (handValue >= 400000000)
                return "Straight";
            else if (handValue >= 300000000)
                return "Three of a Kind";
            else if (handValue >= 200000000)
                return "Two Pair";
